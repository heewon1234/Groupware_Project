...
<link rel="stylesheet" type="text/css" href="/css/commons/common.css">
</head>
									...
									<td class="jt_div">
										<c:forEach items="${jobTitleList}" var="job">
											<c:if test="${job.job_rank eq i}">
												<span class="parent_span"> <span><c:out value="${job.position}" /></span> <a class="remove_jt_btn btn hidden">X</a>
												</span>
											</c:if>
										</c:forEach>
									</td>
				...
				<form action="/job/insertJobRole">
					<div id="jr_contents">
						<c:forEach var="job" items="${jobRoleList}">
							<span class="parent_span"> <span>${job.job_name}</span> <a class="remove_jr_btn btn hidden">X</a>
							</span>
						</c:forEach>
					</div>
					<button type="submit">저장</button>
